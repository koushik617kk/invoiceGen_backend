...
class CAScheduling(Base):
    __tablename__ = "ca_scheduling"
    
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    invoice_id = Column(Integer, ForeignKey("invoices.id"), nullable=True)  # First invoice that triggered this
    ...
    # CA Details
    ca_name = Column(String, default="CA Rajesh Kumar")
    ca_phone = Column(String, default="+91-XXXXX-XXXXX")
    ca_email = Column(String, default="ca@invoicegen.com")
    ...
    def __repr__(self):
        return f"<CAScheduling(user_id={self.user_id}, status='{self.status}')>"
...
class SubscriptionPlan(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String, unique=True, nullable=False)  # 'free_trial', 'paid'
    display_name = Column(String, nullable=False)  # 'Free Trial', 'Paid Plan'
    price_monthly = Column(Float, default=0.0)  # 0 for free trial, 158 for paid
    trial_days = Column(Integer, default=14)  # 14 days for free trial
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class UserSubscription(Base):
    ...
    trial_start_date = Column(DateTime, nullable=True)  # When trial started (first login)
    trial_end_date = Column(DateTime, nullable=True)  # When trial ends
    next_billing_date = Column(DateTime, nullable=True)  # Next payment due
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class SubscriptionPayment(Base):
    ...
    amount = Column(Float, nullable=False)  # 158.0
    ...
    def __repr__(self):
        return f"<SubscriptionPayment(user_id={self.user_id}, amount={self.amount}, status='{self.status}')>"
