...
async def get_current_user(
    ...
    if DEV_AUTH_BYPASS:
        ...
        if not user:
            user = User(cognito_sub="dev-sub", email="dev@example.com", full_name="Dev User", phone="+91-9999999999")
            db.add(user)
            db.commit()
            db.refresh(user)
        return user
    ...
    if not user:
        user = User(cognito_sub=sub, email=email, full_name=name, phone=phone)
        db.add(user)
        db.commit()
        db.refresh(user)
    return user
